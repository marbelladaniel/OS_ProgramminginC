...
tree_t *make_tree(int value) {
  tree_t *t = malloc(sizeof(tree_t)); //the malloc/calloc(for an array) call is what indicates heap allocation, this way the valued are stored even after the funtion is returned 
  t->value = value;
  t->left_child = NULL;
  t->right_child = NULL;
  return t;
